...
class TransactionProcessor {
    fun addTransactionMenu(addTransaction: (Double, Category) -> Boolean, type: TransactionType) {
        ...
        ) println("Transaction added successfully") else println("Unsupported operation: Negative Balance")
    }
    ...
    fun viewTransaction(transactionRepository: TransactionRepository, type: TransactionType) {
        ...
        if (transactionDetails != "No ${type.name.lowercase()} transactions found."){
            print(
            ...
            when (readln().toIntOrNull()) {
                ...
                2 -> deleteTransaction(transactionRepository, type)
                3 -> return
    ...
    private fun deleteTransaction(transactionRepository: TransactionRepository, type: TransactionType) {
        print("Enter Transaction ID: ")
        val transactionId: Int = readln().toInt()
        val isTransactionDeleted = transactionRepository.deleteTransaction(transactionId, type)
    ...
    private fun editTransaction(transactionRepository: TransactionRepository, type: TransactionType) {
        ...
        when (readln().toIntOrNull()) {
            1 -> {
                ...
                val editTransactionAmount = transactionRepository.editTransactionAmount(transactionId, amount, type)
            ...
            2 -> {
                ...
                val editTransactionCategory =
                    transactionRepository.editTransactionCategory(
                        transactionId,
                        transactionCategories[transactionIndex - 1],
                        type
                    )
    ...
    private fun readlnNumber(message: String): Double {
